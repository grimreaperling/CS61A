...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if score > running_high:
            stat  = str(score) + " point(s)! That's a record gain for Player " + str(who) + "!"
            print(stat)
            return announce_highest(who, return_score, score)
        else:
            return announce_highest(who, return_score, running_high)
    return say
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    def calculate(*args):
        ...
        while k <= trials_count:
            k, result = k + 1, result + original_function(*args)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max, k = averaged_roll_dice(1, dice), 2 
    index = 1
    while k <= 10:
        number = averaged_roll_dice(k, dice)
        if number > max:
            max, index = number, k        # note: don't repeat the test in the singel loop it will change the result
        k = k + 1
    return index
    # END PROBLEM 9
