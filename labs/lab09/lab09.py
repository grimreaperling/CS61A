...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        lst = insert_into_all(s[0], subseqs(s[1:])) + subseqs(s[1:])
        return lst
...
def insert(link, value, index):
    ...
    """
    if index == 0:
        curr = Link(link.first, link.rest)
        link.first = value
        link.rest = curr
        return 
    curr = link
...
def long_paths(t, n):
    ...
    if t.is_leaf():
        ...
        result = []
        for b in t.branches:
            result += long_paths(b, n - 1)
        return insert_into_all(t.label, result)
...
def reverse_other(t):
    ...
    def helper(t, k):
        if t.is_leaf():
            return
        if k % 2 == 1:
            new = [b.label for b in t.branches][::-1]
            i = 0
            while i < len(t.branches):
                t.branches[i].label = new[i]
                i = i + 1
        for b in t.branches:
            helper(b, k + 1)
    helper(t, 1)
