...
def distance(city_a, city_b):
    ...
    5.0
    """
    mid = (get_lon(city_a) - get_lon(city_b)) ** 2 + (get_lat(city_a) - get_lat(city_b)) ** 2
    return sqrt(mid)
...
def add_trees(t1, t2):
    ...
    """
    trees = []
    val = label(t1) + label(t2)
    if is_leaf(t1):
        return tree(val, branches(t2))
    if is_leaf(t2):
        return tree(val, branches(t1))
    for i, j in zip(branches(t1), branches(t2)):
        trees += [add_trees(i, j)]
    if len(branches(t1)) > len(branches(t2)):
        for i in branches(t1)[len(branches(t2)):]:
            trees += [i]
    if len(branches(t2)) > len(branches(t1)):
        for i in branches(t2)[len(branches(t1)):]:
            trees += [i]
    return tree(val, trees)
