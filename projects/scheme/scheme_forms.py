...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        validate_form(binding, 2, 2)            #ensure each clause contains exact two element
        names = Pair(binding.first, names)       

        validate_formals(names)                 #ensure without the repitition of the assignment

        res = scheme_eval(binding.rest.first, env)
        vals = Pair(res, vals)

        bindings = bindings.rest                #enter the next loop
    # END PROBLEM 14
