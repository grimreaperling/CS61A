...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if self.is_container:
            if self.ant_contained != None and not self.ant_contained.is_double:
                self.ant_contained.damage *= 2
                self.ant_contained.is_double = True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        if p.is_hive:
            return None
        while True: 
            if p.bees != [] and self.lower_bound <= k <= self.upper_bound:
                return random_bee(p.bees)
            else:
                if p.entrance != None and not p.entrance.is_hive: 
                    p = p.entrance
                    k += 1
                else:
                    return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if amount >= self.health:
            harm = harm + self.damage
        for bee in list(self.place.bees):
            bee.reduce_health(harm)
        super().reduce_health(amount)
        # END Problem 5

# BEGIN Problem 6
class WallAnt(Ant):
    """WallAnt to protect other ants"""
    ...
    def __init__(self, health=4):
        """Creat a Ant with a HEALTH quantity."""
        super().__init__(health)
# END Problem 6

# BEGIN Problem 7
class HungryAnt(Ant):
    """HungryAnt that eat a random number of bees every 3 turns"""
