...
class Coin:
    ...
    def worth(self):
        extra = Mint.present_year - self.year
        if extra > 50:
            extra -= 50
        else:
            extra = 0
...
def two_list(vals, counts):
    ...
    """
    lnk = Link(vals[0])
    curr, count = lnk, counts[0]
    while count > 1:
        curr.rest = Link(vals[0])
        curr = curr.rest
        count -= 1
    if len(vals) > 1:
...
class VirFib():
    ...
    def next(self):
        if self.value == 0:
            ...
            next = VirFib(self.previous + self.value)
...
def is_bst(t):
    ...
    if len(t.branches) > 2:
        return False
    if len(t.branches) == 1 or bst_min(t.branches[0]) <= t.label < bst_max(t.branches[1]):
        return False not in [is_bst(b) for b in t.branches]
    else:
        return False
