...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    lst = [str for str in paragraphs if select(str)]
    if k < len(lst):
        return lst[k]
    else:
        return ''
...
def about(topic):
    ...
    def filter(s):
        ...
        lst = split(s)
        lst = [lower(word) for word in lst]
        for word in topic:
            if word in lst:
                return True
        return False
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    if limit < min([diff_function(typed_word, word, limit) for word in word_list]):
        return typed_word
    result = min(word_list, key=lambda word:diff_function(typed_word, word, limit))
    return result
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    lst = [[timestamps[i + 1] - timestamps[i] for i in range(len(timestamps) - 1)] for timestamps in times_per_player]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    lst = [[] for palyer in get_all_times(match)]
    k = 0
    while k < len(get_all_words(match)):
        i = 0
        while i < len(get_all_times(match)):
            if time(match, i, k) == min([time(match, player, k) for player in range(len(get_all_times(match)))]):
                lst[i] += [get_word(match, k)]
                break
            i = i + 1
        k = k + 1
    return lst
