...
class Player:
    def __init__(self, deck, name):
        ...
        self.deck = deck
        self.name = name
        self.hand = [self.deck.draw() for i in list(range(5))]
...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        if player.hand == []:
            return
        card = player.hand[0]
        card_copy = Card(card.name, card.attack, card.defense)
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if player.hand == []:
            return
        k = 0
        highest, i = -float('inf'), 0
        while i < len(player.hand):
            curr = player.hand[i].power(opponent_card) 
            if curr > highest:
                highest = curr
                k = i
            i += 1
